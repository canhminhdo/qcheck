...
bool StormRunner::isAvailable() {
    ...
    if (stormPath == nullptr) {
        std::cerr << Tty(Tty::RED) << "Error: " << Tty(Tty::RESET) << "Environment variable STORM_PATH is not set." <<
                std::endl;
        return false;
    }
    ...
    if (access(getPath().c_str(), F_OK) == -1) {
        std::cerr << Tty(Tty::RED) << "Error: " << Tty(Tty::RESET) <<
                "Could not find the Storm model checker in STORM_PATH." << std::endl;
        return false;
    }
    return true;
}

void StormRunner::printTiming(const Timer &timer) {
    if (Configuration::showTiming) {
        Int64 real;
        Int64 virt;
        Int64 prof;
        if (timer.getTimes(real, virt, prof)) {
            std::cout << "Time for model checking with Storm in " << prof / 1000 << "ms cpu (" << real / 1000 <<
                    "ms real)";
        }
    }
    std::cout << "\n";
